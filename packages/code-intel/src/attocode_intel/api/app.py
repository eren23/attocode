...
async def _lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    if config.is_service_mode:
        from attocode_intel.db.engine import dispose_engine, init_engine
        ...
        try:
            ...
            raise
    async with app.state.mcp_transport.manager.run():
        yield
    await app.state.mcp_transport.gateway.close()

    # Shutdown debouncer
    from attocode_intel.api.routes.notify import _debouncer

    if _debouncer is not None:
        await _debouncer.shutdown()

    if config.is_service_mode:
        from attocode_intel.db.engine import dispose_engine
...
def create_app(config: CodeIntelConfig | None = None) -> FastAPI:
    ...
    from attocode_intel.api import deps
    from attocode_intel.api.middleware import MetricsMiddleware, RequestLoggingMiddleware
    from attocode_intel.api.routes import (
        adr,
        analysis,
        files,
        graph,
    ...
    from attocode_intel.api.mcp_transport import MCPTransport
    ...
    if config.is_service_mode:
        from attocode_intel.api.middleware import RateLimitMiddleware

        app.add_middleware(RateLimitMiddleware, requests_per_minute=300)

    # Core routes — each module has router_v1 (text) + router_v2 (JSON)
    from attocode_intel.api.routes.operations import router as operations_router
    ...
    if os.path.isdir(static_dir):
        ...
        async def spa_fallback(full_path: str) -> FileResponse:
            ...
            if file_path.is_relative_to(Path(static_dir).resolve()) and file_path.is_file():
