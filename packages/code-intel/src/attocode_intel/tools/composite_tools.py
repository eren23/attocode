...
def _find_test_files_by_convention(file_path: str, project_dir: str) -> list[str]:
    ...
    if ext in (".py",):
        ...
        candidates.extend([
            ...
            # tests/unit/ mirror
            "tests/unit/" + file_path.removeprefix("src/").replace(
                f"{basename}.py", f"test_{basename}.py"
            ),
...
def review_change(
    ...
    # Analysis failures must not be presented as successful verification.
    incomplete = any("error:" in section.lower() for section in report_sections)
    # Assessment
    if incomplete:
        assessment = "Review incomplete: one or more analysis passes failed. Resolve the reported errors before relying on this review."
    elif total_findings == 0:
        assessment = "No findings were reported by the completed checks. This does not establish that the change is safe."
...
def architecture_drift(
    ...
    """
    from attocode_intel._internal.integrations.context.architecture_drift import (
        check_drift,
        format_report,
    )
