...
from attocode_intel._shared import _get_project_dir, mcp
...
def _get_registry() -> RuleRegistry:
    ...
    from attocode_intel.request_context import current_request, scoped_store
    if current_request.get() is not None:
        return scoped_store("rules", _build_scoped_registry)
    ...
    with _registry_lock:
        ...
        from attocode_intel.rules.registry import RuleRegistry
        from attocode_intel.rules.loader import load_builtin_rules, load_user_rules
        from attocode_intel.rules.packs.pack_loader import load_all_packs
        from attocode_intel.rules.plugins.plugin_loader import load_all_plugins
        from attocode_intel.rules.hygiene import apply_persistent_disable
        from attocode_intel.rules.profiling import FeedbackStore
...
def _collect_files(
    ...
    from attocode_intel.rules.executor import _EXT_LANG
...
def _analyze_impl(
    ...
    from attocode_intel.rules.formatter import format_findings
...
def list_rules(
    ...
    from attocode_intel.rules.formatter import format_rules_list
...
def list_packs() -> str:
    ...
    """
    from attocode_intel.rules.packs.pack_loader import list_example_packs
    from attocode_intel.rules.formatter import format_packs_list
...
def install_pack(name: str) -> str:
    ...
    """
    from attocode_intel.rules.packs.pack_loader import install_pack as _install
...
def register_rule(yaml_content: str) -> str:
    ...
    from attocode_intel.rules.loader import _parse_yaml_rule
    from attocode_intel.rules.model import RuleSource
...
def test_rules(
    ...
    """
    from attocode_intel.rules.testing import (
        RuleTestRunner,
        format_test_report,
    )
...
def ci_scan(
    ...
    """
    from attocode_intel.rules.ci import (
        CIConfig, CIRunner, format_ci_summary, format_github_annotations,
    )
    from attocode_intel.rules.sarif import findings_to_sarif, sarif_to_json
    from attocode_intel.rules.model import RuleSeverity
...
def rule_stats(rule_id: str = "") -> str:
    ...
    """
    from attocode_intel.rules.profiling import (
...
def evolve_rules(
    ...
    """
    from attocode_intel.rules.evolution import EvolutionState, evolve
...
def synthesize_rule(
    ...
    """
    from attocode_intel.rules.synthesis import (
        render_rule_yaml,
...
def rule_hygiene(
    ...
    """
    from attocode_intel.rules.hygiene import (
        apply_hygiene,
        compute_hygiene,
...
def search_community_packs(
    ...
    """
    from attocode_intel.rules.marketplace import search_packs, format_registry_search
...
def install_community_pack(
    ...
    """
    from attocode_intel.rules.marketplace import (
        install_remote_pack,
        search_packs,
    )
...
def validate_pack_tool(pack_path: str = "") -> str:
    ...
    """
    from attocode_intel.rules.marketplace import validate_pack, prepare_pack_for_publish
...
def import_rules(
    ...
    if source_format.lower() == "semgrep":
        from attocode_intel.rules.importers.semgrep import convert_semgrep_to_yaml
        return convert_semgrep_to_yaml(content)
    else:
        return f"Unsupported format '{source_format}'. Supported: semgrep"


def _build_scoped_registry(project_dir):
    from attocode_intel.rules.registry import RuleRegistry
    from attocode_intel.rules.loader import load_builtin_rules, load_user_rules
    from attocode_intel.rules.packs.pack_loader import load_all_packs
    from attocode_intel.rules.hygiene import apply_persistent_disable
    ...
    registry.register_many(load_builtin_rules())
    _, rules = load_all_packs(project_dir)
    registry.register_many(rules)
    registry.register_many(load_user_rules(project_dir))
    from attocode_intel.request_context import current_request
    request = current_request.get()
    if request and request.source == "local":
        from attocode_intel.rules.plugins.plugin_loader import load_all_plugins
        _, plugin_rules = load_all_plugins(project_dir)
        registry.register_many(plugin_rules)
    apply_persistent_disable(registry, FeedbackStore(project_dir))
    return registry
