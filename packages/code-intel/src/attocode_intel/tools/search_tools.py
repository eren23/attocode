...
from attocode_intel._internal.integrations.context.frecency import FrecencyResult
...
def _get_project_dir() -> str:
    """Dynamic dispatch to :func:`attocode_intel._shared._get_project_dir`.
    ...
    """
    from attocode_intel import _shared
...
def _get_semantic_search() -> SemanticSearchManager:
    ...
    if (request := current_request.get()) is not None:
        return request.service._get_semantic_search()
...
def _get_security_scanner() -> SecurityScanner:
    ...
    from attocode_intel.request_context import current_request
    if (request := current_request.get()) is not None:
        return request.service._get_security_scanner()
...
def _get_trigram_index() -> TrigramIndex | None:
    ...
    if current_request.get() is not None:
        def build(root):
            import os
            from attocode_intel._internal.integrations.context.trigram_index import TrigramIndex
            index = TrigramIndex(index_dir=os.path.join(root, ".attocode", "index"))
            return index if index.load() else None
        return scoped_store("trigrams", build)
...
def dataflow_scan(
    ...
    """
    from attocode_intel._internal.integrations.security.dataflow import analyze_project, format_report
...
def fast_search(
    ...
    if trigram_idx is not None and trigram_idx.is_ready():
        if explain:
            from attocode_intel._internal.integrations.context.trigram_index import QueryResult
