...
class CodeIntelService:
    ...
    def readiness_report_data(
        ...
        from attocode_intel.readiness import ReadinessEngine
    ...
    def readiness_report(
        ...
        *,
        ...
        from attocode_intel.readiness import ReadinessEngine, ReadinessSeverity
    ...
    def code_evolution(self, path: str, symbol: str = "", since: str = "", max_results: int = 20) -> str:
        from attocode_intel.tools.history_tools import code_evolution as _code_evolution
    ...
    def recent_changes(self, days: int = 7, path: str = "", top_n: int = 20) -> str:
        from attocode_intel.tools.history_tools import recent_changes as _recent_changes
    ...
    def hotspots_data(self, top_n: int = 15) -> dict:
        ...
        from attocode_intel.helpers import (
    ...
    def graph_dsl_data(self, query: str) -> dict:
        ...
        """
        from attocode_intel.graph_query_parser import (
            GraphQueryExecutor,
            GraphQueryParser,
        )
    ...
    def community_detection_data(
        ...
        try:
            from attocode_intel.community import louvain_communities as _louvain
            communities, modularity_score = _louvain(all_files, adj, weights)
            method = "louvain"
        except ImportError:
            from attocode_intel.community import bfs_connected_components
    ...
    def distill_data(
        ...
        if level == "signatures":
            from attocode_intel.tools.distill_tools import _extract_signatures

            text = _extract_signatures(ast_cache, selected, max_tokens)
        elif level == "structure":
            from attocode_intel.tools.distill_tools import _build_structure
    ...
    def code_evolution_data(
        ...
        """
        from attocode_intel.tools.history_tools import (
            _filter_by_symbol,
    ...
    def recent_changes_data(
        ...
        from attocode_intel.tools.history_tools import _run_git
    ...
    def dead_code_data(
        ...
        """
        from attocode_intel.tools.dead_code_tools import (
            _find_dead_files,
            _find_dead_modules,
            _find_dead_symbols,
        )
    ...
    def bootstrap(self, task_hint: str = "", max_tokens: int = 8000, indexing_depth: str = "auto") -> str:
        ...
        if size_tier == "small":
            ...
            hs_text = "Use hotspots for a focused complexity report after indexing completes."
            sections.append(f"## Repository Map\n{map_text}")
    ...
    def dead_code(
        ...
        from attocode_intel.tools.dead_code_tools import dead_code as _dead_code
    ...
    def distill(
        ...
        from attocode_intel.tools.distill_tools import distill as _distill
    ...
    def repo_map_ranked(self, task_context: str = "", token_budget: int = 1024, exclude_tests: bool = True) -> str:
        from attocode_intel.tools.analysis_tools import repo_map_ranked as _repo_map_ranked
    ...
    def bug_scan(self, base_branch: str = "main", min_confidence: float = 0.5) -> str:
        from attocode_intel.tools.analysis_tools import bug_scan as _bug_scan
    ...
    def fast_search(
        ...
        from attocode_intel.tools.search_tools import fast_search as _fast_search
    ...
    async def lsp_enrich(self, files: list[str]) -> str:
        from attocode_intel.tools.lsp_tools import lsp_enrich as _lsp_enrich
    ...
    def record_adr(
        ...
        from attocode_intel.tools.adr_tools import record_adr as _record_adr
    ...
    def list_adrs(self, status: str = "", tag: str = "", search: str = "") -> str:
        from attocode_intel.tools.adr_tools import list_adrs as _list_adrs

        return _list_adrs(status=status, tag=tag, search=search)

    def get_adr(self, number: int) -> str:
        from attocode_intel.tools.adr_tools import get_adr as _get_adr

        return _get_adr(number)

    def update_adr_status(self, number: int, status: str, superseded_by: int | None = None) -> str:
        from attocode_intel.tools.adr_tools import update_adr_status as _update_adr_status
    ...
    def analyze(
        ...
        from attocode_intel.tools.rule_tools import _analyze_impl
    ...
    def list_rules(
        ...
        from attocode_intel.tools.rule_tools import list_rules as _list_rules
    ...
    def list_packs(self) -> str:
        from attocode_intel.tools.rule_tools import list_packs as _list_packs

        return _list_packs()

    def register_rule(self, yaml_content: str) -> str:
        from attocode_intel.tools.rule_tools import register_rule as _register_rule

        return _register_rule(yaml_content=yaml_content)

    def install_pack(self, name: str) -> str:
        from attocode_intel.tools.rule_tools import install_pack as _install_pack
