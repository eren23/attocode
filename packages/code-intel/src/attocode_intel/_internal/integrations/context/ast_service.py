...
from attocode_intel._internal.integrations.context.cross_references import (
    CrossRefIndex,
    SymbolLocation,
    SymbolRef,
)
...
class ASTService:
        ...
        if is_full_parse:
            ...
            if self._store:
                ...
                for rel, ast_item in self._ast_cache.items():
                    try:
                        mtime = os.path.getmtime(os.path.join(self._root_dir, rel))
                    except OSError:
                        continue
                    stored_files.append(self._build_stored_file(rel, ast_item, mtime))
                if stored_files:
                    self._store.save_files_batch(stored_files)
                for rel in self._ast_cache:
                    self._index.persist_file(rel)
        ...
        if dep_graph:
            for src, targets in dep_graph.forward.items():
                for target in targets:
                    self._index.add_file_dependency(src, target)
            state.dep_graph_files = len(dep_graph.forward)
        self._initialized = True
        return state
    ...
    def _run_hydration(self) -> None:
        ...
        if self._store:
            try:
                for offset in range(0, len(cache_snapshot), 100):
                    if self._hydration_stop.is_set():
                        return
                    batch = []
                    for rel, ast in cache_snapshot[offset:offset + 100]:
                        try:
                            mtime = os.path.getmtime(os.path.join(self._root_dir, rel))
                        except OSError:
                            continue
                        batch.append(self._build_stored_file(rel, ast, mtime))
                    self._store.save_files_batch(batch)
                    for stored in batch:
        ...
        state.dep_graph_files = len(dep_graph.forward) if dep_graph else 0
        state.phase = "ready"
