...
class SemanticSearchManager:
    ...
    def _get_frecency_score(self, file_path: str) -> float:
        ...
        if self._frecency_tracker is None:
            try:
                from attocode_intel._internal.integrations.context.frecency import FrecencyTracker
    ...
    def _ensure_provider(self) -> None:
        ...
        if self._provider is not None:
            return
        from attocode_intel._internal.integrations.context.embeddings import (
        ...
        if not self._keyword_fallback:
            from attocode_intel._internal.integrations.context.vector_store import (
                VectorStore,
        ...
        if self.nl_mode == "heuristic" and self._summarizer is None:
            from attocode_intel.indexing.summarizer import get_summarizer
    ...
    def queue_reindex(self, file_path: str) -> None:
        """Queue changes only for an already initialized embedding index.

        File notifications must never initialize or download an embedding model.
        Explicit indexing or search initializes the provider before enqueueing.
        """
    ...
    def _supported_languages() -> set[str]:
        ...
        try:
            from attocode_intel._internal.integrations.context.ts_parser import supported_languages
    ...
    def search(
        ...
        from attocode_intel._internal.integrations.context.ast_chunker import reciprocal_rank_fusion
    ...
    def _rerank(
        ...
        """
        from attocode_intel._internal.integrations.context.reranker import get_reranker
