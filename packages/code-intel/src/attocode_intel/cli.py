...
def _print_help() -> None:
    print(
        ...
        "  amazon-q            Amazon Q Developer (~/.aws/amazonq/mcp.json)\n"
        "  copilot-cli         GitHub Copilot CLI (~/.copilot/mcp-config.json)\n"
        "  junie               Junie / JetBrains (.junie/mcp/mcp.json)\n"
        "  kiro                Kiro (.kiro/settings/mcp.json)\n"
        "  trae                Trae (.trae/mcp.json)\n"
        "  firebase            Firebase Studio (.idx/mcp.json)\n"
        "  amp                 Amp / Sourcegraph (.amp/settings.json)\n"
        "  continue            Continue.dev (.continue/mcp.json)\n"
...
def _cmd_install(args: list[str]) -> None:
    from attocode_intel.installer import ALL_TARGETS_STR, install, install_hooks
...
def _cmd_uninstall(args: list[str]) -> None:
    from attocode_intel.installer import ALL_TARGETS_STR, uninstall, uninstall_hooks
...
def _cmd_serve(args: list[str], *, debug: bool = False) -> None:
    ...
    from attocode_intel._shared import enable_remote_if_configured
    if local_only:
    ...
    from attocode_intel.server import (
        _start_file_watcher,
        _start_queue_poller,
        _stop_file_watcher,
        mcp,
...
def _cmd_status() -> None:
    ...
    from attocode_intel.installer import _get_user_config_dir
    ...
    from attocode_intel.installer import _find_command
...
def _cmd_probe_install(args: list[str]) -> None:
    ...
    from attocode_intel.installer import ALL_TARGETS_STR
    from attocode_intel.probe import probe_install
...
def _cmd_bundle(args: list[str]) -> None:
    ...
    subcmd = args[0]
    if subcmd == "export":
        from attocode_intel.bundle import export_bundle
    ...
    if subcmd == "inspect":
        ...
        from attocode_intel.bundle import inspect_bundle
...
def _cmd_notify(args: list[str]) -> None:
    ...
    from attocode_intel.config import load_remote_config
...
def _cmd_connect(args: list[str]) -> None:
    ...
    """
    from attocode_intel.config import RemoteConfig, save_remote_config
...
def _cmd_setup(args: list[str]) -> None:
    ...
    try:
        ...
        print(
            ...
            f"  source {env_dev} && uvicorn attocode_intel.api.app:create_app "
    ...
    print(
        ...
        f"  source {env_dev} && python -m attocode_intel.workers.run\n\n"
        f"  # Frontend (optional)\n"
...
def _cmd_gc(args: list[str]) -> None:
    ...
    from attocode_intel.config import CodeIntelConfig, load_remote_config
